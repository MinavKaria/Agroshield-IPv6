...
# import tensorflow_hub as hub
model = keras.layers.TFSMLayer("./plant_2_dis", call_endpoint='serving_default')
...
class_indices = ""
with open("./class_indices.json") as fp:
    class_indices = json.load(fp)
print(class_indices)
# class_indices_url = "./class_indices.json"
...
def predict():
    ...
    try:
        ...
        predictions = predictions['dense'][0]

        # Get the predicted class index
        pred_ind = tf.argmax(predictions).numpy()
        print(pred_ind)
        # Get the corresponding class label
        predicted_label = class_indices[str(pred_ind)]
